...
def visualize_results(
    ...
    plot_highway_geometries.plot(
        ax=ax, label="Austrian highway network", color="black", zorder=0
    )

    for ij in range(0, len(bounds) - 1):
        cat = plot_results_and_geom_df[
            plot_results_and_geom_df["total_charging_pole_number"].isin(
                np.arange(bounds[ij] + pole_peak_cap, bounds[ij + 1] + pole_peak_cap)
                / pole_peak_cap
            )
        ]
        plt.scatter(
            cat["x"].to_list(),
            cat["y"].to_list(),
            s=np.array(cat["total_charging_pole_number"].to_list()) * fact,
            color=colors[ij],
            label=str(int(bounds[ij]) + pole_peak_cap)
            + " - "
            + str(int(bounds[ij + 1]))
            + " kW",
            # edgecolors='black',
        ...
        print(
            ij,
            len(cat),
            np.array(cat["total_charging_pole_number"].to_list()) * fact,
            colors[ij],
        )
    ...
    plt.text(
        1.1e6,
        5.8e6,
        "Nb. charging stations: "
        + str(int(optimization_result[0]))
        + "\nNb. charging poles: "
        + str(int(optimization_result[1]))
        + "\nTotal infrastructure capacity (kW): "
        + str(int(optimization_result[1]) * pole_peak_cap),
        fontsize=14,
    )
