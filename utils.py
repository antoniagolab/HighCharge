...
def retrieve_tc_data_for_all_segments_for_dir(
    ...
    return pd.DataFrame(
        {"dir_0": traffic_counter_df.tc_dir_0.to_list(), "dir_1": traffic_counter_df.tc_dir_1.to_list()}
    )
...
def calculate_demand(pois_df, tc_df, sp_demand, col_tc_dir_0, col_tc_dir_1, std=5000):
    ...
    for seg_id in segment_ids:
        ...
        if len(extract_tc) > 0:
            ...
            poi_extract_dir_0, poi_extract_dir_1 = split_by_dir(poi_extract, 'dir', reindex=False)
...
def append_children_pois_on_same_segm(poi_id, segment_id, direction, pois_gdf, dist, last_name, counter, segm_tree,
    ...
    if direction == 1:
        extract_pois = extract_pois.sort_values(by="dist_along_segment", ascending=False)
        dists = extract_pois.dist_along_segment.to_list()
        extract_pois["dist_along_segment"] = [0] + [dists[-1] - d for d in dists[-1:0:-1]]
...
def append_children_pois(pois_gdf, segment_id, dir, dist, last_c, counter, variable_dic):
    """
    ...
    # find this node
    if dir == 1:
        extract_seg_gdf = extract_seg_gdf.sort_values(by="dist_along_segment", ascending=False)
        dists = extract_seg_gdf.dist_along_segment.to_list()
        extract_seg_gdf["dist_along_segment"] = [0] + [dists[-1] - d for d in dists[-1:0:-1]]
...
def append_for_last_link(io_rels, seg_id, direction, pois_df_0, pois_df_1, links_gdf, segments_gdf):
    """
...
def retrieve_demand_split_ratios(segment_id, direction, pois_df, links_gdf, segments_gdf, poi_dir_0, poi_dir_1):
    """
...
def are_ras_along_the_way(ij, seg_id, direction, path, pois_df_0, pois_df_1, kl, end_seg_id, end_direction, n0, n1,
    ...
    if seg_id == end_seg_id:
        ...
        if end_direction == 0 and link_id == segments_gdf[segments_gdf.ID == end_seg_id].link_0.to_list()[0]:
            end_extract = end_extract.sort_values(by="dist_along_segment", ascending=False)
        elif end_direction == 1 and link_id == segments_gdf[segments_gdf.ID == end_seg_id].link_1.to_list()[0]:
