...
def calculate_dist_max(av_driving_range):
    ...
    :return:
    """
    return av_driving_range * (4 / 5) * 0.6 * 1000
...
def create_frequency_df(df):
    ...
    df[column_starttime] = pd.to_datetime(
        df[column_starttime], format="%H:%M:%S"
    ).dt.time
    ...
    for ij in range(0, 96):
        # collect frequencies
        new_datetime = (
            datetime.datetime.combine(datetime.date.today(), current_time) + timedelta
        )
        next_time = new_datetime.time()
        extract_df = df[
            (df[column_starttime] >= current_time) & (df[column_starttime] < next_time)
        ]
        frequency_df = frequency_df.append(
            {"time": current_time, "frequency": extract_df[column_factor].sum()},
            ignore_index=True,
        )
...
def calculate_max_util_params(dist_max):
    ...
    :return: mu, gamma_h
    """
    way_data = pd.read_csv(
        "mobility_data\ÖU_2013-2014_Datensatz_CSV_Version/ÖU_2013_2014_Wegedatensatz.csv"
    )
    ...
    h_t = car_drivers_data[
        (
            (car_drivers_data[column_trip_time] > 45)
            & (car_drivers_data[column_trip_distance] > 25)
        )
    ...
    for ij in range(0, 96):
        # upper bound
        top_datetime = datetime.datetime.combine(
            datetime.date.today(), current_time
        ) + datetime.timedelta(hours=1)
        top_time = top_datetime.time()
        extract_df_h_t = h_t_counts[
            (h_t_counts.index >= current_time) & (h_t_counts.index < top_time)
        ]
